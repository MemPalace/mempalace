...
### Improvements
- **`sweep` tags drawers with a wing and room.** The message-level transcript sweeper previously wrote drawers without taxonomy metadata, leaving every swept transcript in the `unknown` bucket and invisible to wing-scoped queries and the palace graph views. `sweep()`/`sweep_directory()` now accept keyword-only `wing`/`room` (stamped onto drawer metadata when provided; legacy metadata shape unchanged when omitted), and the CLI gains `--wing`/`--room` flags whose defaults follow the `mine` convention: wing = normalized source directory basename (for a file target, its parent directory), room = `conversations`. Names are validated via `normalize_wing_name` + `sanitize_name` with a clean error exit on invalid input.
