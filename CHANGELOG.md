...
- **A CLI search never loads a second local HNSW copy after discovering a live matching Hub.** The original Hub-forwarding path returned to direct local search when the Hub capability was stale, search configuration drifted, authentication was missing or rejected, the registered endpoint failed its health check, or the request could not be represented exactly. On a multi-gigabyte palace, even a transient health race therefore recreated the duplicate-index memory outage the forwarder was introduced to prevent. Once a live per-palace registration exists, those conditions now fail closed with a sanitized repair diagnostic; direct search remains available only when no live matching Hub is registered, or through the explicit `MEMPALACE_HUB_FORWARD=0` escape hatch after accepting the memory cost. Follow-up to #2379.
- **HTTP conversation mines no longer starve every Hub reader for the full ingest.** The HTTP transport classified `mempalace_mine` as one request-wide palace write, so scanning, normalizing and chunking a large live transcript held the exclusive Hub lock even though those phases never mutate Chroma; a 420 MB Codex JSONL kept the writer path occupied for more than an hour while searches and peer tools queued behind it. Conversation mines now keep one mutation owner for their complete crash-safe lifecycle, share backend reads with peer searches, and take the Hub's exclusive access lock only around bounded collection-open/upsert/update/delete bursts. Other mutations cannot interleave between mine batches, project/extract modes retain their request-wide lock, and the write-stall watchdog measures actual Chroma bursts instead of ordinary parsing time. Builds on the incremental, lossless re-mine contract from #2408 and closes defect 5 of #2403.
- **Codex Stop hooks recognize the current `item_completed` conversation format.** Codex CLI 0.149 and newer moved user and agent turns from `payload.type=user_message|agent_message` into typed `item_completed` records. Transcript normalization understood the new records, but the Stop hook's message counter and checkpoint summarizer still saw an empty session, so an ordinary session that never compacted could end without either capture path running. Normalization and hook accounting now share one Codex event parser, including multi-block text extraction and exclusion of reasoning, command, and file-change items. Text blocks retain their original leading/trailing whitespace and indentation instead of being stripped independently.
- **A failed diary checkpoint stays pending when transcript mining is disabled.** `_ingest_transcript` previously reported the deliberate `hooks.mine_transcript=false` no-op as a successful dispatch, so `hook_stop` advanced its checkpoint marker even when the diary write beside it failed. Disabled ingestion now has a distinct result and cannot acknowledge an unsaved checkpoint. The pending checkpoint's exact entry, message window and idempotency key are written atomically to hook state and reused unchanged until acknowledgement; the frozen payload is removed only after the per-session last-save marker itself is durably advanced, so an ack/write-marker crash window cannot rebuild different content under the same key.
- **A daemon liveness race no longer acknowledges an unsaved Stop-hook interval.** The hook probes daemon health once, then submits its diary and transcript jobs. If the daemon disappeared between that probe and the transcript enqueue, the submit exception was logged but returned the same status as a confirmed job; when the diary also failed, `hook_stop` advanced its marker even though neither capture path saved anything. A failed enqueue now stays pending without an eager direct fallback, and the next Stop retries through the transcript job's dedupe key.
- **Hook-to-Hub diary forwarding stays inside the hook budget and retries exactly once logically.** A live but wedged Hub could hold `hook_stop` synchronously for 60 seconds even though the harness kills the hook after 30 seconds and the repository budget is 500 ms, preventing the transcript path from being dispatched at all. The redundant synchronous `/healthz` round-trip is gone; live server registration is followed by one authenticated diary POST whose complete `urlopen` / response-read / close lifecycle runs in a daemon worker behind an absolute 350 ms deadline, so a Hub cannot evade it by drip-feeding headers or body bytes within each socket timeout. Hook checkpoints also carry a stable session/epoch/boundary idempotency key through direct, daemon and Hub routes: the persisted boundary cannot change when new prompts arrive during a failed interval, and SessionEnd advances a durable epoch before clearing the throttle marker so a later resume cannot reuse an older checkpoint identity. `mempalace_diary_write` maps that key to one deterministic entry and upserts it, then removes physical chunks from the prior representation that the replacement no longer uses. A committed write whose response was lost can therefore be retried without creating a second timestamped entry, overwriting a prior resume's history, or leaving suffix chunks from a longer first attempt readable as current content.
- **Failed orphan cleanup keeps a rewritten transcript visibly incomplete.** A shrink/rewrite writes its new target set before deleting drawer ids the source no longer produces. If that last delete failed transiently, the target rows had already received a complete current-mtime marker, so every later mine skipped the unchanged file and left the old drawers searchable forever. Rewrites now withhold the current marker until cleanup succeeds; a failed delete leaves the source retryable, and a successful retry finalizes the whole target set without re-embedding unchanged chunks.
- **Changed conversation chunks preserve the prior verbatim generation until every replacement batch succeeds.** Deterministic per-position drawer IDs made incremental comparison cheap, but writing changed text back under the same ID overwrote the old document one batch at a time; a later batch failure therefore left neither the old nor the new transcript complete. Changed chunks now embed under a content-addressed physical generation linked to their stable logical position. These rows remain marked staged; vector and backend-lexical queries exclude them before applying top-K, sqlite BM25 excludes them before its candidate limit, and even the unfiltered HNSW-mismatch fallback progressively refills until it has enough committed rows. Content-hash prefetch ignores staged rows too, so an invisible crash-left generation cannot suppress a committed copy under another path. A single durable per-source commit marker is the crash-atomic visibility switch: before it, only the old generation is eligible; after it, the complete staged token is eligible even if the process dies during metadata finalization or stale-ID cleanup. Markers are scoped to their Chroma collection and extraction mode, and cleanup-pending itself triggers the next mine even when every stale row was already deleted and only the final marker update was interrupted. Search, lexical union, list, get, update and delete resolve every physical generation through its stable `logical_drawer_id`, treating marker-token membership—not `filed_at`—as the authority when content reverts to an older physical row. Candidate queries refill until that active row is present; pending old/new duplicates collapse to one logical result, and delete removes every visible physical sibling. `list_drawers` applies visibility rules before totals and pagination, so neither marker records nor unpublished generations leak as user memories.
- **Re-mining a changed transcript is incremental, and an interrupted mine can no longer shrink the palace.** The convo miner's rebuild contract was purge-before-refile: any changed source had ALL of its drawers deleted, then the whole file re-embedded. An active Claude Code session appends to its own transcript every turn, so with hook auto-save the same session was fully purged and fully re-mined at every save point — O(n²) over the life of a session, >5 minutes per pass on a 75 MB transcript — and a crash or kill inside the refile left the palace holding less than before the mine started (observed live: 5,765 of 8,022 drawers after one SIGTERM), the exact crash-loss the "Incremental only" design principle forbids. Drawer ids were already deterministic over `(source_file, extract_mode, chunk_index)`, so the pass now snapshots what the palace holds, re-embeds only chunks whose stored `chunk_hash` differs (drawers from before this field count as changed once and migrate themselves), refreshes unchanged drawers with a metadata-only touch so the `chunk_total` completion check still sees one full current-mtime group, and deletes orphaned ids last, after the full new set is in place. A pass interrupted anywhere leaves every pre-existing drawer present and the group incomplete, so the next mine repairs instead of skipping (#2183); the worst crash window leaves transient duplicates, never loss. (#2403)
- **A Stop-hook save whose diary write failed no longer re-fires the transcript mine on every subsequent Stop.** The save marker advanced only on diary success, while the expensive transcript ingest ran unconditionally before it. On the shared-brain setup — a hub holding the writer lease for its lifetime, refusing the hook's in-process diary write — the marker therefore never advanced, and EVERY Stop re-triggered a full mine of the transcript. The marker now advances when the diary landed or a transcript mine/follow-up was durably dispatched. If the same-target mine is already active, one detached watcher coalesces a latest-state follow-up behind it instead of assuming the old snapshot covers newly appended turns. A transcript that vanished or became unreadable after message counting reports undispatched, preserving retry-on-total-failure. (#2403)
- **Hook diary checkpoints reach a palace served by a hub.** A long-lived `mempalace serve` holds the writer lease for its whole process lifetime, which `cli._forward_mine_to_hub` already accounts for: the hook's transcript mine spawns the CLI, and the CLI hands the job to the hub over HTTP. The checkpoint took the other path — `hooks_cli._save_diary_direct` imports `mcp_server.tool_diary_write` and calls it in-process — and no hub forwarding existed there, so on exactly the machines the shared-brain guide tells you to set up, every checkpoint was refused with `palace ... is held by PID <hub>` while the raw mine beside it succeeded. `_save_diary_direct` now discovers a live writable registration and sends one deadline-bounded, token-rotating `mempalace_diary_write` call; once a matching Hub exists, an unconfirmed response is never retried in-process. Honors `MEMPALACE_HUB_FORWARD=0` and skips read-only hubs. (#2403)
