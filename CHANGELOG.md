...
- **Windows drive-letter transcript paths now retain their project wing in fallback routing.** `_wing_from_transcript_path` accepts Claude Code's `[a-zA-Z]--` drive-prefix encoding alongside the existing POSIX form, so a transcript without usable `cwd` metadata no longer falls back to `wing_sessions`. (#2304)
