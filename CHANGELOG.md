...
- **Current Codex rollout turns are counted and mined by every shipped hook path.** Codex 0.147+ writes canonical conversation turns as `event_msg` / `item_completed` records with `UserMessage` / `AgentMessage` items instead of the legacy `user_message` / `agent_message` payloads. The normalizer, Python hooks, and portable shell-hook helper now share one parser for both schemas, preserve mixed bare-string/text-block content, and continue to ignore duplicate or synthetic `response_item` records. Sessions that were never filed can be backfilled with `mempalace mine ~/.codex/sessions/ --mode convos`; unchanged sessions already filed through the raw fallback remain skipped at normalization version 2 and need a future scoped migration. (#2230)
