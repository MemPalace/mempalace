...
- **A Stop-hook save whose diary write failed no longer re-fires the transcript mine on every subsequent Stop.** The save marker advanced only on diary success, while the expensive transcript ingest ran unconditionally before it. On the shared-brain setup — a hub holding the writer lease for its lifetime, refusing the hook's in-process diary write — the marker therefore never advanced, and EVERY Stop re-triggered a full mine of the transcript: the mine occupied the palace write path, which kept the diary failing, which kept re-triggering the mine, until the hub answered nothing else (observed live as `TRIGGERING SAVE at exchange 5193` repeating turn after turn, with the hub's own starvation warning logged 48 times). The marker now advances when the diary landed OR the ingest is in flight — `_spawn_mine` reports "already running" as in-flight for the same reason — and stays put only when nothing succeeded, preserving the retry-on-total-failure property. (#2403)
