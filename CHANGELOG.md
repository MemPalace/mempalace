...
- **Concurrent first use no longer lets one palace split across replica identities.** Two processes could both observe a missing `replica.json`, share the same fixed temp file, and overwrite the canonical identity, allowing a caller to stamp operations with an ID different from the file left on disk or fail with `FileNotFoundError`. First-mint candidates now use private sibling temps and atomic no-clobber publication; losers adopt the persisted winner and failed publication leaves no partial identity. Filesystems without hard-link support fail closed with an actionable explanation instead of silently falling back to an unsafe overwrite. (#2274)
