...
- **Hook-to-Hub diary forwarding stays inside the hook budget and retries exactly once logically.** A live but wedged Hub could hold `hook_stop` synchronously for 60 seconds even though the harness kills the hook after 30 seconds and the repository budget is 500 ms, preventing the transcript path from being dispatched at all. Hub discovery, health and authenticated diary forwarding now share an absolute 350 ms deadline; response-body reading runs behind that wall-clock deadline, so a Hub cannot evade it by drip-feeding bytes within each socket timeout. Hook checkpoints also carry a stable content/session idempotency key through direct, daemon and Hub routes; `mempalace_diary_write` maps that key to one deterministic entry and upserts it, then removes physical chunks from the prior representation that the replacement no longer uses. A committed write whose response was lost can therefore be retried without creating a second timestamped entry or leaving suffix chunks from a longer first attempt readable as current content.
