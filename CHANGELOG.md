...
- **Changed conversation chunks preserve the prior verbatim generation until every replacement batch succeeds.** Deterministic per-position drawer IDs made incremental comparison cheap, but writing changed text back under the same ID overwrote the old document one batch at a time; a later batch failure therefore left neither the old nor the new transcript complete. Changed chunks now embed under a content-addressed physical generation linked to their stable logical position. All old physical generations remain untouched through every new batch, superseded generations are deleted only after the complete target set exists, and a crash-left generation is reused on retry instead of embedded again.
