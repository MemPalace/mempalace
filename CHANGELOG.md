...
- **macOS LaunchAgent template for `mempalace serve`, and why `ProcessType` matters.** Adds `deploy/com.mempalace.server.plist` alongside the systemd unit. A LaunchAgent with no `ProcessType` is CPU and I/O throttled by launchd, which presents as a slow server rather than as a launchd policy.
