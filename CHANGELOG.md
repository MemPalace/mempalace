...
- **`repair-status` no longer reports a large HNSW ghost surplus as healthy flush lag.** When bulk or repeated deletes leave substantially more persisted HNSW elements than live SQLite embeddings, the capacity probe now reports `STALE`, explains that deleted entries may degrade vector results, and recommends rebuilding from SQLite. Small surpluses inside the configured delete-lag tolerance remain `OK`, and the stale-surplus state does not activate the native-crash read/write fence reserved for an undersized HNSW index. (#2176)
