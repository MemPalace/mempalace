...
def _kimi_user_prompt(text):
    return json.dumps(
        {"type": "turn.prompt", "input": [{"type": "text", "text": text}], "origin": {"kind": "user"}, "time": 1}
    )
    ...
    return json.dumps(
        {
            ...
            "message": {"role": "user", "content": [{"type": "text", "text": text}], "origin": {"kind": kind}, "toolCalls": []},
            "time": 1,
        }
    )
...
def test_kimi_jsonl_filters_injection_messages():
    ...
    lines = [
        ...
        _kimi_user_message("<system-reminder>permission mode active</system-reminder>", kind="injection"),
...
def test_kimi_jsonl_skips_metadata_and_tool_events():
    ...
    lines = [
        ...
        json.dumps({"type": "context.append_loop_event", "event": {"type": "tool.call", "name": "Read", "args": {}}}),
