...
class TestSearchMemories:
    ...
    def test_search_memories_vector_path_uses_explicit_collection_name(self):
        mock_col = MagicMock()
        # _count_in_scope reads .count() on unfiltered scopes; without an
        # explicit return_value, MagicMock returns a MagicMock that breaks
        # the `available_in_scope > vector_hit_count` comparison.
        mock_col.count.return_value = 0
        mock_col.query.return_value = {
            "documents": [[]],
            "metadatas": [[]],
    ...
    def test_effective_distance_clamped_to_valid_cosine_range(self):
        ...
        # _count_in_scope calls .count() on unfiltered scopes; without an
        # explicit return_value, MagicMock returns a MagicMock that breaks
        # the `available_in_scope > vector_hit_count` comparison in
        # search_memories' "more in scope" warning.
        drawers_col.count.return_value = 2
...
class TestSearchCLI:
    ...
    def test_search_query_error_degrades_to_warning(self, fake_palace_path, capsys):
        ...
        is returning fewer results than expected. Uses ``fake_palace_path``
        so the State A/B filesystem checks pass through to the mocked
        backend (#1498)."""
        mock_col = MagicMock()
        mock_col.count.return_value = 0
