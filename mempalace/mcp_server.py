...
# Legacy full-scan cache — kept as a nil-initialised global so tool_mine can
# invalidate it after writes and so upstream callers (non-chroma backend paths,
# test monkeypatches) have a stable name to reference.  The actual metadata
# read path now routes through palace_stats (SQL aggregation, #1657); this
# variable is no longer populated by the read tools.
