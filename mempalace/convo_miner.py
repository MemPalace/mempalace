...
def _source_file_stored_drawers(
    collection, source_file: str, extract_mode: str
) -> dict:
                ...
                try:
                    if upsert_ids:
                        ...
                        for batch_start in range(
                            0, len(upsert_ids), DRAWER_UPSERT_BATCH_SIZE
                        ):
                            slice_end = min(batch_start + DRAWER_UPSERT_BATCH_SIZE, len(upsert_ids))
                            assert_no_collisions(list(zip(upsert_ids[batch_start:slice_end], upsert_metas[batch_start:slice_end])), collection)
                            try:
                                collection.upsert(
