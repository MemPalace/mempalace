...
Returns a ChromaDB-compatible embedding function using one of three execution
paths: a local ONNX model bound to a user-selected ONNX Runtime provider,
PyTorch + sentence-transformers on Apple Metal, or an OpenAI-compatible HTTP
...
For ``minilm``, the backend can be either ONNX Runtime (default) or, for
the ``mps`` device specifically, PyTorch + sentence-transformers. Switching
the device for that model does not invalidate an existing palace — the same
model weights and vector space are used either way. The MPS path currently
supports ``minilm`` only: ``embeddinggemma`` falls back to its CPU ONNX path,
and ``openai-compat`` ignores the local device setting.
...
Runtime's CoreML provider explicitly.
...
def _resolve_providers_for_model(device: str, model: str) -> tuple[list, str]:
    """Resolve a local device while enforcing model/backend compatibility."""
    normalized_model = (model or "minilm").strip().lower()
    if normalized_model == "openai-compat":
        return ([], "openai-compat")

    providers, effective = _resolve_providers(device)
    if effective == "mps" and normalized_model == "embeddinggemma":
        # _build_mps_ef only has a verified build for all-MiniLM-L6-v2.
        # Silently taking the MPS path here would build MiniLM vectors under
        # an EF the palace expects to be embeddinggemma-shaped — a vector-space
        # correctness bug, not merely a missed speedup.
...
def get_embedding_function(device: Optional[str] = None, model: Optional[str] = None):
    ...
    providers, effective = _resolve_providers_for_model(device, model)
...
def describe_device(device: Optional[str] = None, model: Optional[str] = None) -> str:
    ...
    if device is None or model is None:
        ...
        if device is None:
            device = cfg.embedding_device
    _, effective = _resolve_providers_for_model(device, model)
    return effective
