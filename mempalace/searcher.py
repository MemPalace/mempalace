...
# Search tunables. Kept at module level so the dispatcher reads cleanly
# and tests can patch these without surgery on a function body.
CLOSET_RANK_BOOSTS = [0.40, 0.25, 0.15, 0.08, 0.04]
CLOSET_DISTANCE_CAP = 1.5  # cosine dist > 1.5 = too weak to use as signal
MAX_HYDRATION_CHARS = 10000

# Over-fetch ratios. The hybrid rerank pool is ``n_results * RATIO`` so
# BM25/closet signals can reorder a wider candidate set than the final cut.
_VECTOR_OVERFETCH_RATIO = 3
_CLOSET_OVERFETCH_RATIO = 2
_BM25_UNION_OVERFETCH_RATIO = 3
...
def _metadata_filter_sql(row_id_expr: str, wing: str, room: str) -> tuple[str, list[str]]:
    """Build ``AND EXISTS(SELECT ... FROM embedding_metadata ...)`` clauses for wing/room.

    The metadata is stored as one row per (embedding_id, key, value), so each
    filter joins back to ``embedding_metadata`` via the row id expression
    supplied by the caller (e.g. ``e.id`` or ``embedding_fulltext_search.rowid``).
...
def _select_via_fts5(
    ...
    """FTS5 MATCH selector. Returns candidate row ids.

    Raises ``sqlite3.Error`` on tokenizer mismatch / syntax error so the
    caller can distinguish "FTS5 hit nothing" (return [] and stop) from
    "FTS5 broke" (fall through to the recency selector).
    """
    filter_sql, filter_params = _metadata_filter_sql("embedding_fulltext_search.rowid", wing, room)
    rows = conn.execute(
        f"""
...
def _select_via_recency(
    ...
    """``ORDER BY created_at DESC`` selector. Raises ``sqlite3.Error`` on schema mismatch.

    Used as the second fallback when there are no usable FTS tokens or
    FTS5 itself errored.
    """
    filter_sql, filter_params = _metadata_filter_sql("e.id", wing, room)
    rows = conn.execute(
        f"""
        SELECT e.id
        FROM embeddings e
...
def _select_via_id(
    ...
    """``ORDER BY e.id DESC`` selector — the final fallback.

    Used when even the recency-window query errors (legacy palaces without
    ``created_at``, partial restores missing ``segments`` rows, etc.).
    """
    filter_sql, filter_params = _metadata_filter_sql("e.id", wing, room)
    rows = conn.execute(
        f"""
...
def _select_bm25_candidate_ids(
    conn,
    query: str,
    collection_name: str,
    wing: str,
    ...
    try:
        ...
        logger.debug("recency-window query failed; trying id-ordered fallback", exc_info=True)

    try:
        return _select_via_id(conn, collection_name, wing, room, max_candidates)
    except sqlite3.Error:
        logger.debug("id-ordered fallback also failed", exc_info=True)
        return []


def _load_drawer_metadata(conn, ids: list) -> list:
    """Fetch ``(id, key, string_value, int_value)`` rows for the given embedding ids."""
    if not ids:
        return []
...
def _shape_drawer_candidates(meta_rows: list, wing: str, room: str) -> list:
    """Group flat metadata rows into per-drawer dicts, apply Python-side wing/room filter.

    FTS5 candidates may include entries from other wings (the metadata
    EXISTS-clauses restrict at JOIN time but distinct shared metadata rows
    can still leak through), so re-checking in Python is the safety net.
    """
    drawers: dict = {}
        ...
        candidates.append(
            {
                ...
                # unless _include_internal is set.
...
def _rank_and_trim(candidates: list, query: str, n_results: int, _include_internal: bool) -> list:
    """BM25-score candidates, sort descending, trim to ``n_results``, strip internals.

    Mutates each candidate to add ``bm25_score``; uses a transient ``_score``
    field for sorting (popped before return). Internal source-path/chunk
    fields are stripped unless the caller opted in via ``_include_internal``
    (the union-merge path does this for chunk-precise dedup).
    """
...
def _bm25_only_via_sqlite(
    ...
    try:
        candidate_ids = _select_bm25_candidate_ids(
            conn, query, collection_name, wing, room, max_candidates
        )
        ...
        meta_rows = _load_drawer_metadata(conn, candidate_ids)
    finally:
        conn.close()

    candidates = _shape_drawer_candidates(meta_rows, wing, room)
    hits = _rank_and_trim(candidates, query, n_results, _include_internal)
...
def _merge_bm25_union_candidates(
    ...
    try:
        bm25_extra = _bm25_only_via_sqlite(
            ...
            wing=wing,
            room=room,
            n_results=n_results * _BM25_UNION_OVERFETCH_RATIO,
...
def _build_closet_boost_lookup(
    palace_path: str,
    query: str,
    where: dict,
    n_results: int,
) -> dict:
    """Query the closets collection and return ``{source_file: (rank, dist, preview)}``.

    Closets are best-effort: any failure (missing collection, query error, etc.)
    yields an empty dict so the caller degrades to drawer-only ranking. This
    preserves the "closets are a signal, never a gate" invariant.
    """
    ...
    try:
        ...
        ckwargs = {
            "query_texts": [query],
            "n_results": n_results * _CLOSET_OVERFETCH_RATIO,
            "include": ["documents", "metadatas", "distances"],
        }
        if where:
            ckwargs["where"] = where
...
def _score_drawer_candidates(
    drawer_results,
    closet_boost_by_source: dict,
    max_distance: float,
    n_results: int,
) -> list:
    """Score drawer candidates with closet boost + cosine clamp; sort and trim.

    The ordinal closet signal ("which closet matched best") is more reliable
    than absolute distance on narrative content where closet distances cluster
    in the 1.2–1.5 range regardless of match quality. Boost is capped at
    ``CLOSET_DISTANCE_CAP`` and follows the ``CLOSET_RANK_BOOSTS`` schedule.

    Returns the top ``n_results`` hits, each carrying both public fields and
    the internal ``_sort_key`` / ``_source_file_full`` / ``_chunk_index`` keys
    that downstream enrichment and cleanup expect.
    """
    scored: list = []
...
def _hydrate_closet_boosted_hits(hits: list, drawers_col, query: str) -> None:
    """Replace each closet-boosted hit's text with the keyword-best chunk + neighbors.

    For every hit whose ``matched_via == "drawer+closet"``, fetch all drawers
    from the same source file, find the chunk with the most query-term
    matches, and join it with one neighbor on each side. The closet said
    "this source is relevant"; vector may have picked the wrong chunk within
    it; this picks the right one.

    Mutates ``hits`` in place: sets ``text``, ``drawer_index``, and
    ``total_drawers``. Truncates oversized hydrations to ``MAX_HYDRATION_CHARS``.
    Skips drawer-only hits and any source with one or zero drawers.
    """
    for h in hits:
...
def _strip_internal_hit_fields(hits: list) -> None:
    """Remove the leading-underscore internal keys before returning hits."""
    for h in hits:
...
def search_memories(
    ...
    # GATE — direct drawer search is always the baseline. This avoids the
    # "weak-closets regression" where narrative content produces low-signal
    # closets and closet-first routing would hide drawers that direct search
    # would find.
    try:
        dkwargs = {
            "query_texts": [query],
            "n_results": n_results * _VECTOR_OVERFETCH_RATIO,
            "include": ["documents", "metadatas", "distances"],
        }
        if where:
            dkwargs["where"] = where
    ...
    closet_boost_by_source = _build_closet_boost_lookup(palace_path, query, where, n_results)
    hits = _score_drawer_candidates(drawer_results, closet_boost_by_source, max_distance, n_results)
    _hydrate_closet_boosted_hits(hits, drawers_col, query)

    # Candidate strategy hook: default "vector" is a no-op; "union" merges
    # top-K BM25 candidates from sqlite. ``max_distance`` is forwarded so
    # union mode can refuse to inject BM25-only (distance=None) candidates
    # that would silently bypass the caller's strict distance threshold.
    ...
    _strip_internal_hit_fields(hits)
