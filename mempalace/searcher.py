...
def search(query: str, palace_path: str, wing: str = None, room: str = None, n_results: int = 5):
    ...
    """
    # State guard before delegating: _open_collection_or_explain prints the
    # state-specific message (States A–E, including the #1498 lazy-create
    # short-circuit) and returns None. The handle is kept only to resolve
    # the display metric below — search_memories opens its own.
    ...
    # Delegate to search_memories so CLI and MCP callers share the same
    # hybrid ranking, sqlite-BM25 fallback (this PR), and scope-aware
    # warnings. Remaining structured errors (backend mismatch, races)
    # are mapped through the error path below.
    ...
    # Hits are already built and hybrid-reranked (metric-aware, per RFC 001)
    # by search_memories(); the delegate path centralizes retrieval,
    # BM25-sqlite fallback, and legacy-metric warning so CLI and MCP callers
    # share a single source of truth. The metric is resolved here only to
    # label the similarity scores the delegate already computed.
    ...
    for i, hit in enumerate(hits, 1):
        ...
        # ``similarity`` was computed by the delegate via the metric-aware
        # _distance_to_similarity (per RFC 001); label it with the actual
        # metric rather than assuming cosine. Fallback hits carry
        # similarity=None (no vector score) and print BM25-only.
        if similarity is not None and bm25 is not None:
            print(f"      Match:  {metric}_sim={similarity}  bm25={bm25}")
        elif similarity is not None:
            print(f"      Match:  {metric}_sim={similarity}")
