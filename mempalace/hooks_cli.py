            ...
            try:
                _submit_daemon_job(
                    "mine",
                    {
                        "source": str(path),
                        "mode": "convos",
                        # Same alias-aware wing the spawn path below uses — a
                        # hardcoded "sessions" here would make wing_aliases.json
                        # silently inert for anyone running the hooks daemon.
                        "wing": wing,
                        "agent": "mempalace",
                    },
...
def _basename_to_wing(basename: str, aliases: Optional[dict] = None) -> str:
    ...
    if prefix_match:
        return prefix_match
    return f"wing_{_safe_wing_slug(basename)}"
