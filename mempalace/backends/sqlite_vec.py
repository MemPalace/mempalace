...
def _load_sqlite_vec():
    ...
    try:
        ...
        raise ImportError(
            "sqlite_vec is required for sqlite_vec backend. "
            "Install it with: pip install mempalace[sqlite_vec]"
        ) from exc
...
def _pushdown_sql(clause: dict) -> Optional[tuple[str, list[Any]]]:
    """Translate one flat metadata clause into a JSON1 SQL predicate.

    Returns ``None`` for anything JSON1 cannot evaluate with the same
    semantics as :func:`_matches_where` — bare logical operators, unsupported
    operators, ``None`` operands, keys outside the identifier charset — so the
    caller can fall back to Python-side filtering.
    """
    if not clause:
        return ("1", [])
    parts: list[str] = []
    ...
    for key, expected in clause.items():
        ...
        if not re.fullmatch(r"[a-zA-Z0-9_-]+", str(key)):
            return None
        path = f"json_extract(metadata_json, '$.{key}')"
        if isinstance(expected, dict):
            for op, operand in expected.items():
                if op == "$eq":
                    parts.append(f"{path} = ?")
                    ...
                    # _compare() coerces both sides with float(), so compare
                    # CAST-ed REALs here too — raw SQL would order every TEXT
                    # value above every number, where the Python filter does
                    # numeric coercion (float() failure == no match).
                    sql_op = {"$gt": ">", "$gte": ">=", "$lt": "<", "$lte": "<="}[op]
                    parts.append(f"CAST({path} AS REAL) {sql_op} CAST(? AS REAL)")
                    params.append(operand)
                elif op in ("$in", "$nin"):
                    negation = "NOT " if op == "$nin" else ""
                    if not isinstance(operand, list) or not operand:
                        return None
                    placeholders = ", ".join(["?"] * len(operand))
                    parts.append(f"{path} {negation}IN ({placeholders})")
    ...
    return ("(" + " AND ".join(parts) + ")", params) if parts else ("1", [])
...
class SQLiteVecCollection(BaseCollection):
    ...
    def _ensure_vec_table(self, cur, dim: int) -> None:
        ...
        and prevent cross-collection vector corruption. Rows are keyed by the
        document id (vec0 text primary key): ``documents`` stays the single
        source of truth and no parallel rowid mapping is kept in sync.
        """
        table_name = self._vec_table()
        meta_key = f"vec0_table_created:{self._collection_name}"
        row = cur.execute("SELECT value FROM meta WHERE key = ?", (meta_key,)).fetchone()
        if row and row[0] == "1":
            if self._vec_table_is_legacy(cur, table_name):
                self._rebuild_vec_table(cur, table_name, dim)
            return
        cur.execute(f"DROP TABLE IF EXISTS {table_name}")
        self._create_vec_table(cur, dim)
        cur.execute("INSERT OR REPLACE INTO meta(key, value) VALUES (?, '1')", (meta_key,))

    def _create_vec_table(self, cur, dim: int) -> None:
        cur.execute(
            f"CREATE VIRTUAL TABLE {self._vec_table()} USING vec0("
            f"  doc_id text primary key,"
            f"  embedding float[{dim}] distance_metric=cosine"
            f")"
        )

    def _vec_table_is_legacy(self, cur, table_name: str) -> bool:
        """True when the vec table predates the doc_id primary key shape."""
        try:
            cur.execute(f"SELECT doc_id FROM {table_name} LIMIT 0")
        except sqlite3.OperationalError:
            return True
        return False

    def _rebuild_vec_table(self, cur, table_name: str, dim: int) -> None:
        """Rebuild a rowid-keyed (pre-merge) vec table as doc_id-keyed.

        documents.embedding holds the authoritative vector blob, so the
        rebuild is lossless and runs inside the caller's transaction.
        """
        cur.execute(f"DROP TABLE IF EXISTS {table_name}")
        self._create_vec_table(cur, dim)
        cur.execute(
            f"INSERT INTO {table_name} (doc_id, embedding) "
            "SELECT id, embedding FROM documents WHERE collection_id = ?",
    ...
    def add(self, *, documents, ids, metadatas=None, embeddings=None):
        ...
        with self._cursor() as cur:
            ...
            for doc_id, doc, meta, emb_blob, dim in prepared:
                ...
                cur.execute(
                    "INSERT INTO " + self._vec_table() + " (doc_id, embedding) VALUES (?, ?)",
                    (doc_id, emb_blob),
                )
    ...
    def upsert(self, *, documents, ids, metadatas=None, embeddings=None):
        ...
        with self._cursor() as cur:
            ...
            for doc_id, doc, meta, emb_blob, dim in prepared:
                ...
                cur.execute(
                    "DELETE FROM " + self._vec_table() + " WHERE doc_id = ?",
                    (doc_id,),
                )
                cur.execute(
                    "INSERT INTO " + self._vec_table() + " (doc_id, embedding) VALUES (?, ?)",
    ...
    @staticmethod
    def _json1_available(cur) -> bool:
        try:
            cur.execute("SELECT json_extract('{}', '$.x')")
        except sqlite3.OperationalError:
            return False
        return True

    @staticmethod
    def _where_conjuncts(where: dict) -> Optional[list[dict]]:
        """Flat conjunction equivalents of ``where``.

        Returns ``None`` when the filter uses ``$or`` (or nested logic) that a
        single AND-of-predicates cannot express, so the caller falls back to
        Python-side evaluation.
        """
        if any(key in _SUPPORTED_OPERATORS for key in where):
            if set(where) == {"$and"} and all(isinstance(clause, dict) for clause in where["$and"]):
                out: list[dict] = []
                for clause in where["$and"]:
                    sub = SQLiteVecCollection._where_conjuncts(clause)
                    if sub is None:
                        return None
                    out.extend(sub)
                return out
    ...
    def _knn_candidates(self, cur, collection_id: int, q_blob: bytes, k: int) -> list[tuple]:
        """Unfiltered ANN candidates: (doc_id, distance, document, metadata_json, embedding)."""
        try:
            return cur.execute(
                f"SELECT v.doc_id, v.distance, d.document, d.metadata_json, d.embedding "
                f"FROM {self._vec_table()} v "
                f"JOIN documents d ON d.collection_id = ? AND d.id = v.doc_id "
                f"WHERE v.embedding MATCH ? AND k = ? ORDER BY v.distance",
                (collection_id, q_blob, int(k)),
            ).fetchall()
        except sqlite3.OperationalError as exc:
            if "no such table" in str(exc) or "no such module" in str(exc):
                # vec0 table not created yet (no documents written)
                return []
            raise

    def _filtered_query_candidates(
        self,
        cur,
        collection_id: int,
        q_blob: bytes,
        ...
        """Filtered candidates via an exact distance ranking over ``documents``.

        Scalar metadata predicates are pushed down into JSON1
        (``json_extract``) when the where-dialect allows it; everything else
        ($or, $contains, malformed JSON, document filters) is evaluated in
        Python. Ranking always covers the whole collection, so a matching row
        can never fall outside a bounded ANN window — the k-expansion miss
        this replaces.
        """
        _validate_where(where)
        _validate_where(where_document)
        needs_python_filter = False
        pushdown: Optional[tuple[str, list[Any]]] = None
        if where:
            conjuncts = self._where_conjuncts(where)
            if conjuncts is None or not self._json1_available(cur):
                needs_python_filter = True
            else:
                built = [_pushdown_sql(clause) for clause in conjuncts]
                if any(part is None for part in built):
                    needs_python_filter = True
                else:
                    parts = [part[0] for part in built]
                    flat_params = [p for part in built for p in part[1]]
                    if parts:
                        pushdown = (" AND ".join(parts), flat_params)

        sql = (
            "SELECT id, document, metadata_json, embedding, "
            "vec_distance_cosine(embedding, ?) AS _dist "
            "FROM documents WHERE collection_id = ?"
        )
        params: list[Any] = [q_blob, collection_id]
        if pushdown is not None:
            sql += " AND " + pushdown[0]
            params.extend(pushdown[1])

        if needs_python_filter or where_document:
            out: list[tuple] = []
            for doc_id, doc, meta_json, emb_blob, dist in cur.execute(
                sql + " ORDER BY rowid", params
            ):
                if not _matches_where(_json_loads(meta_json), where):
                    continue
                if not _matches_where_document(doc or "", where_document):
                    continue
                out.append((doc_id, float(dist), doc, meta_json, emb_blob))
                if len(out) >= n_results:
                    break
            return out
        rows = cur.execute(sql + " ORDER BY _dist LIMIT ?", params + [int(n_results)]).fetchall()
        return [
            (doc_id, float(dist), doc, meta_json, emb_blob)
            for doc_id, doc, meta_json, emb_blob, dist in rows
        ]
        ...
        with self._cursor() as cur:
            ...
            for query_vector in query_embeddings:
                ...
                if where is None and where_document is None:
                    vec_rows = self._knn_candidates(
                        cur, collection_id, q_blob, max(n_results * 3, 10)
                    )
                else:
                    vec_rows = self._filtered_query_candidates(
                        cur, collection_id, q_blob, n_results, where, where_document
                    )

                for doc_id, distance, doc_text, meta_json, emb_blob in vec_rows:
                    ids_out.append(doc_id)
                    if spec.documents:
                        docs_out.append(doc_text or "")
                    if spec.metadatas:
                        metas_out.append(_json_loads(meta_json))
                    if spec.distances:
    ...
    def delete(self, *, ids=None, where=None):
        with self._cursor() as cur:
            ...
            if ids is None:
                ...
                ids = [row["id"] for row in rows]
            # A created-but-never-written collection has no vec0 table yet.
            vec_table_exists = (
                cur.execute(
...
class SQLiteVecBackend(BaseBackend):
        ...
        if vec_mod:
            if not hasattr(conn, "enable_load_extension"):
                raise ImportError(
                    ...
                    "support, so sqlite-vec cannot be loaded (common on macOS "
                    "python.org builds — use a Homebrew- or uv-managed Python)"
                )
    ...
    def delete_collection(self, palace: Union[str, PalaceRef], collection_name: str) -> None:
        ...
        with handle.lock:
            ...
            # Drop the vec0 table unconditionally: a never-written collection
            # has none yet, and dropping removes the shadow tables so a
            # recreate with a different dimension cannot reuse stale state.
