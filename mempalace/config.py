...
class MempalaceConfig:
    ...
    def search_config_fingerprint(self) -> str:
        ...
        if embedding_model == "openai-compat":
            ...
            if embedding_model == "generic-onnx":
                # Unlike the bundled e5-small preset, generic-onnx has no
                # fixed model — repo/file/pooling/prefixes/ef_name fully
                # determine it, so a Hub-side vector space change through any
                # of them must be visible in the digest too (#2280).
                effective.update(
                    embedding_onnx_repo=self.embedding_onnx_repo,
                    embedding_onnx_file=self.embedding_onnx_file,
                    embedding_onnx_subfolder=self.embedding_onnx_subfolder,
                    embedding_onnx_max_len=self.embedding_onnx_max_len,
                    embedding_onnx_pooling=self.embedding_onnx_pooling,
                    embedding_onnx_doc_prefix=self.embedding_onnx_doc_prefix,
                    embedding_onnx_query_prefix=self.embedding_onnx_query_prefix,
                    embedding_onnx_ef_name=self.embedding_onnx_ef_name,
                )
        try:
            if backend == "qdrant":
                effective.update(
