...
def cmd_repair(args):
    ...
    # #2562: hold this palace's mine lease across the whole legacy pass. The
    # extraction and the backup copy below take minutes on a large palace and
    # used to run unlocked, so a hook miner starting in that window won the
    # lock and the rebuild then died with MineAlreadyRunning, discarding both
    # the extraction and the backup. rebuild_index and rebuild_from_sqlite
    # already take the lease up front for the same reason; the per-batch
    # acquires inside the rebuild pass through re-entrantly on this thread.
    # Contention is reported before anything is read or copied.
    try:
