...
| Mode | Use when | How |
|---|---|---|
| **Inbox sweep** | Start of every session, and before any long task | `mempalace_event_list` (or `EVENT INBOX`) with `to_agent=<you>`, `since_event_id=<last seen>`, `preview=true` (defaults to newest-first without cursor) |
